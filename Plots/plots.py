...
res = res.replace(to_replace="fsig", value="f-sum sigmoid")
res = res.replace(to_replace="fleaky", value="f-sum leaky")
lab = list(res.head())
...
for _,yuse in enumerate(yvec):
    g = sns.catplot(x="GNN", y=yuse, hue="AGG", kind="bar", data=pattern, ci="sd", capsize=0.1, palette=colors)
